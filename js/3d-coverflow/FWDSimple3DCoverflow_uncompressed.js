...
var FWDS3DCovCategoriesButton = function(
		...
		this.setupMainContainers = function()
		{
			...
			self.textDO.screen.className = "categoriesMenuText";
...
(function(window)
{
	var FWDS3DCovThumb = function(id, data, parent)
	{
		...
		{
			if (FWDS3DCovUtils.isIOS)
			{
				self.mainDO = new FWDS3DCovDisplayObject3D("div", "absolute", "visible");
		...
		this.showThumbIntro2D = function(scale, del)
		{
			...
			if (!FWDS3DCovUtils.hasTransform2d)
			{
				...
				else if (data.thumbsAlignment == "top")
				{
					FWDS3DCovModTweenMax.to(self, .8, {x:Math.floor(self.newX + (self.thumbWidth - scaleW)/2), y:(-Math.floor(parent.thumbsMaxHeight/2)) * scale, ease:Expo.easeOut});
				}
				else
				{
		...
		this.setScale = function(scale, alpha)
		{
			...
			if (!FWDS3DCovUtils.hasTransform2d)
			{
				...
				{
					FWDS3DCovModTweenMax.to(self, .8, {x:Math.floor(self.newX + (self.thumbWidth - scaleW)/2), y:(-Math.floor(parent.thumbsMaxHeight/2)) * scale, w:scaleW, h:scaleH, alpha:alpha, ease:Quart.easeOut});
				}
				else
				{
		...
		this.setScaleInfinite = function(scale)
		{
			...
			if (!FWDS3DCovUtils.hasTransform2d)
			{
				...
				{
					FWDS3DCovModTweenMax.to(self, 0, {x:Math.floor(self.newX + (self.thumbWidth - scaleW)/2), y:(-Math.floor(parent.thumbsMaxHeight/2)) * scale, w:scaleW, h:scaleH, ease:Quart.easeOut});
				}
				else
				{
...
(function(window)
{
	var FWDS3DCovThumbsManager = function(data, parent)
	{
		...
		this.startIntro = function()
		{
			...
			else if (self.data.thumbsAlignment == "top")
			{
				newX = -Math.floor(self.curDataListAr[self.curId].thumbWidth/2);
				newY = -Math.floor(self.thumbsMaxHeight/2);
			}
			else
			{
				newX = -Math.floor(self.curDataListAr[self.curId].thumbWidth/2);
		...
		{	
			...
			if (self.textDO)
			{
				...
				if (self.data.thumbsAlignment == "bottom")
				{
					self.textDO.setY(Math.floor(self.stageHeight/2 + self.thumbsMaxHeight/2 + self.textOffset - (1 - self.scale) * self.thumbsMaxHeight/2));
				}
				else if (self.data.thumbsAlignment == "top")
				{
					self.textDO.setY(Math.floor(self.stageHeight/2 - self.thumbsMaxHeight/2 + self.curDataListAr[self.curId].thumbHeight + self.textOffset - (1 - self.scale) * self.thumbsMaxHeight/2));
				}
				else
				{
					self.textDO.setY(Math.floor(self.stageHeight/2 + self.curDataListAr[self.curId].thumbHeight/2 + self.textOffset - (1 - self.scale) * self.thumbsMaxHeight/2));
				}
			}
		};
		...
		this.setupIntro3D = function()
		{
			...
			for (var i=0; i<self.totalThumbs; i++)
			{
					...
					case "accordion":
						if (i != self.curId)
						{
							if ((self.nrThumbsToDisplay > 0) && (Math.abs(i - self.curId) <= self.nrThumbsToDisplay))
							{
								...
								{
									newX = self.avgThumbWidth*3/4 * Math.abs(i - self.curId) * sgn;
									newZ = -self.avgThumbWidth * Math.sqrt(3)/4 * Math.abs(i - self.curId);
									
									newAngleY = 0;
								}
								else
								{
									newX = (self.avgThumbWidth*3/4 * (Math.abs(i - self.curId)-1) + self.avgThumbWidth*3/4) * sgn;
									newZ = -self.avgThumbWidth * Math.sqrt(3)/4 * Math.abs(i - self.curId);
							...
							else
							{
								if (self.nrThumbsToDisplay > 0)
								{
									...
									if (Math.abs(i - self.curId) % 2 == 0)
									{
										newX = self.avgThumbWidth*3/4 * (self.nrThumbsToDisplay + 1) * sgn;
										newZ = -self.avgThumbWidth * Math.sqrt(3)/4 * (self.nrThumbsToDisplay + 1);
										
										newAngleY = 0;
									}
									else
									{
										newX = (self.avgThumbWidth*3/4 * ((self.nrThumbsToDisplay + 1)-1) + self.avgThumbWidth*3/4) * sgn;
										newZ = -self.avgThumbWidth * Math.sqrt(3)/4 * (self.nrThumbsToDisplay + 1);
				...
				{
					newX = Math.floor(newX) - Math.floor(self.curDataListAr[i].thumbWidth/2);
					newY = Math.floor(newY) - Math.floor(self.thumbsMaxHeight/2);
				}
				else
				{
					newX = Math.floor(newX) - Math.floor(self.curDataListAr[i].thumbWidth/2);
